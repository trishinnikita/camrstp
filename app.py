...
capture_threads = {}
frame_queues = {}
init_lock = threading.Lock()  # Для thread-safe init
        ...
        def capture_loop():
            try:
                ...
                if not cap.isOpened():
                    ...
                    while stream_id in frame_queues:  # Пока очередь существует
                        try:
...
def gen_frames(stream_id):
    ...
    while True:
        try:
            # Lazy init: Создаём очередь и стартуем thread если нет
            if stream_id not in frame_queues:
                rtsp_url = RTSP_URLS.get(stream_id)
                if rtsp_url:
                    start_capture_thread(stream_id, rtsp_url)
                else:
                    raise ValueError(f"No RTSP URL for {stream_id}")
            
            frame = frame_queues[stream_id].get(timeout=1)  # Ждём кадр max 1 сек
        except (queue.Empty, KeyError) as e:
            logging.warning(f"Queue issue for {stream_id}: {e} — using error_frame")
            frame = error_frame
        except Exception as e:
...
# Инициализация стримов (опционально, lazy init покроет)
def init_streams():
    for stream_id, rtsp_url in RTSP_URLS.items():
        if rtsp_url:
            start_capture_thread(stream_id, rtsp_url)
    logging.info("Init_streams completed — all streams ready (or lazy)")

# Gunicorn post_fork hook для инициализации в workers
...
def video_stream(stream_id):
    ...
    try:
        ...
        logging.error(f"Fatal error in video_stream for {stream_id}: {e}")
        # Fallback: Single error image
        load_error_frame()
        ret, buffer = cv2.imencode('.jpg', error_frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
